...
void checkSensorsAlarm() {
  if (s1_ppb_current > s1_max_current || s3_ppb_current > s3_max_current) alarm_current = 1;
  else alarm_current = 0;

  if (s1_ppb_current > s1_max_current) s1_color_current = 1;
  else s1_color_current = 0;
  if (s3_ppb_current > s3_max_current) s3_color_current = 1;
  else s3_color_current = 0;
}

void checkMainSensor() {
  if (s2_ppb_current > s2_max_current) o3_gen_cycle_direction_current = 0;
  else if (s2_ppb_current < s2_min_current) o3_gen_cycle_direction_current = 1;

  if (s2_ppb_current > s2_min_current) start_countdown = 1;

  if (s2_ppb_current <= (s2_max_current /* + s2_ppb_current  * 0.2 */) && s2_ppb_current >= (s2_min_current /* - s2_ppb_current * 0.2 */)) s2_color_current = 1;
  else s2_color_current = 0;
}
...
/*
  ...
  if (Serial2.available()) {
    s2_new_data = true;
    s2_buffer[s2_buffer_counter] = Serial2.read();
    if (s2_buffer_counter < S2_BUFFER_SIZE) s2_buffer_counter++;
    s2_current_millis = millis();
  }
  if (s2_new_data) {
    if ((millis() - s2_current_millis) > 10) {
      s2_new_data = false;
      s2_buffer_counter = 0;
      s2_ppb_current = (s2_buffer[4] * 256) + s2_buffer[5];
    }
  }
  }
*/
...
/*
  ...
  if (Serial3.available()) {
    s3_new_data = true;
    s3_buffer[s3_buffer_counter] = Serial3.read();
    if (s3_buffer_counter < S3_BUFFER_SIZE) s3_buffer_counter++;
    s3_current_millis = millis();
  }
  if (s3_new_data) {
    if ((millis() - s3_current_millis) > 10) {
      s3_new_data = false;
      s3_buffer_counter = 0;
      s3_ppb_current = (s3_buffer[4] * 256) + s3_buffer[5];
